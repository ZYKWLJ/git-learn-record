...
# 七、fork别人的代码
=====
此时为了营造多个登录账号，我需要在同一台电脑上登录不同的git账号。那么需要登录多个github账户。

=====
### 1.fork的含义
Fork 的核心本质是开源协作的 **“安全副本” 机制** —— 让你能在**不影响原仓库的前提下**，自由修改代码、验证想法，再通过 **Pull Request（PR）向原作者贡献你的改动**，是 GitHub 等平台**最核心**的开源协作模式。
先通俗理解：Fork 就像 **“抄作业但不弄脏原作业本”**
假设原仓库是作者的 “正版作业本”：
你直接改原仓库？没权限（除非作者给你写权限）；
你 Fork 之后，**相当于把作业本完整抄到自己账号下，变成 “你的副本”；**
你可以在自己的**副本里随便改（删内容、加功能、改错误），原作者的 “正版” 完全不受影响；**
如果你改得好，还能把 “修改后的部分” 提交给原作者（PR），作者审核后可以把你的改动合并到原仓库。

### 2.如何fork？
fork必须要在网页端改动，git没有相关命令，但是fork之后，我们可以使用git clone命令克隆到本地(注意，此时是git的自己的仓库的，因为fork之后，自己的仓库就有了和原作者仓库相同的代码。如果git clone原作者的仓库，**大概率是没有写权限的！注意！**)，然后在本地修改，最后提交到**自己的**远程仓库。
![fork别人的代码后的结果](image-61.png)

注意，我们push后，只是在自己的仓库改动，绝不是在原作者的仓库。
那如何将自己的push提交到原作者的主线上去？


### 3.PR——将自己的共享代码合并到原作者的主线

- 把本地改动 push 到自己的 Fork 仓库后，打开你 Fork 仓库的 GitHub 网页；
- 点击 “Compare & pull request” 按钮，选择原仓库（ZYKWLJ/git-learn-record）作为 “目标仓库”；
- 填写改动说明，提交 PR；
- 原作者审核你的 PR 后，手动点击 “Merge”（合并），你的改动才会被合并到原仓库；
- 如果原作者不合并，你的改动永远只存在于自己的 Fork 仓库里。
...
# 八、常见的问题
## (一)本地仓库提交时，完成仓库领先于本地提交，导致提交失败，如何解决？

这种情况常见于，当你在本地提交了代码后，发现远程仓库有更新，而你本地的仓库落后于远程仓库，导致提交失败。
例如多人协作时。
### 1.分支不是同一个时
一侧的另一个分支提交了不同，但是导致远程该分支和本地内容不一致
![一侧的另一个分支提交了不同，但是导致远程该分支和本地内容不一致](image-56.png)
尝试提交本地分支的变化，报错，因为分支不一样，本地时main，所以报错服务器内部出错。
![尝试提交本地分支的变化](image-57.png)
显然，本地拉取远程最新的提交，就可以推送了：
![本地拉取远程最新的提交，就可以推送了：](image-58.png)

### 2.分支是同一个时
一侧的同一个分支提交了不同，但是导致远程该分支和本地内容不一致(远处分支高于本地分支)，提交失败
![一侧的同一个分支提交了不同，但是导致远程该分支和本地内容不一致(远处分支高于本地分支)](image-59.png)

拉取后，本地分支就和远程分支一致了：在提交就能成功了！
![拉取后，本地分支就和远程分支一致了：在提交就能成功了！](image-60.png)
