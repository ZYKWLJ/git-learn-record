...
查看第二个提交有误，格式不规整,需要重新修改

![查看第二个提交有误，格式不规整](image-50.png)
使用命令
```
git rebase -i 2c53d4a~1
```
表示重新提交`2c53d4a`这个提交的前一个提交(即要修改的父提交。)
这会重新打开一个vim窗口：
将想要提交的指令前面的pick改为reword，
![将想要提交的指令前面的pick改为reword，紧接着又会和重新打开一个vim窗口，让你重新编辑对应的注释信息](image-51.png)

pick改为reword，表示我要修改这一次提交的注释
![pick改为reword，表示我要修改这一次提交的注释](image-53.png)

紧接着又会和重新打开一个vim窗口，让你重新编辑对应的注释信息
![紧接着又会和重新打开一个vim窗口，让你重新编辑对应的注释信息](image-54.png)
...
### 1.命令
```
git clone https://github.com/ZYKWLJ/git-learn-record.git
//克隆指定路径下的仓库，默认clone全部分支
```
